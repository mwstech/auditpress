...
- **Transport** — `POST /wp-json/pluginlens/v1/mcp/{token}` speaking MCP over JSON-RPC 2.0 (protocol `2025-11-25`, plain JSON responses, stateless). Token-in-path auth behind a swappable interface, rate limiting, failed-auth logging.
...
1. **Tools → PluginLens** in wp-admin: enable the endpoint, generate a token, copy the connection URL.
