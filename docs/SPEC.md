# Auditra: Build Specification v1.0
...
Working name **Auditra**, slug `auditra`. Before anything is written, confirm the slug is free at `https://wordpress.org/plugins/auditra/` (a 404 means available). Fallbacks in order: `plugin-lens`, `pluginscope`, `plugin-radar`.
...
1. Create repo `macronimous/auditra`. **Private for now**, flip to public at Phase 9 before wp.org submission. A community tool needs a public repo; it does not need one while the auth layer is still a guess.
...
```
auditra/
  auditra.php              Main plugin file, header, bootstrap
...
```
POST /wp-json/auditra/v1/mcp/{token}
```
...
- Every function, class, option, transient, and hook prefixed `auditra_` or `Auditra_`.
