# PluginLens: Environment Setup

Two environments, and the second one only matters once.

| Environment | Job | Frequency |
|---|---|---|
| LocalWP site `pluginlens`, a clone of outsourcewebdesign.com | CCD's entire build loop | Constant |
...
1. Create a new site named `pluginlens`. PHP 8.2, WordPress default, whichever web server.
...
3. Set permalinks to Post name. `/wp-json/pluginlens/v1/mcp/{token}` will not resolve on plain permalinks and the token-in-path design breaks entirely.
...
```
cd ~/Local\ Sites/pluginlens/app/public/wp-content/plugins
git clone https://github.com/mwstech/pluginlens.git
```
...
rsync -avz --delete \
  ...
  ./ pluginlens-test:<plugin-path>/pluginlens/
```
...
**Security plugins.** Wordfence, iThemes Security, Sucuri, and All In One WP Security all block REST endpoints by default. Deactivate for the duration or add an exception for the `pluginlens/v1` namespace.
